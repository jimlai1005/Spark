...
class ExplorePublisher:
    ...
    def maybe_publish(self, *, force: bool = False) -> bool:
        ...
        try:
            ...
            if has_version and not force:
                ...
                if reason is not None:
                    self._note_gate("out", reason, now)
                    return False
    ...
    def _note_gate(self, prefix: str, reason: str, now: float) -> None:
        """門檻擋下的共同記帳。<!-- 2026-09-20 第四輪複審 W2 -->：擋下要「會叫」不只
        「可查」（工程原則 #6——進度不推進得有人知道）。冷啟動期間每分鐘擋一次屬
        預期，只在第 1 次與之後每 10 次記一行 warning，避免 journal 洪水。"""
        self._gate_skips += 1
