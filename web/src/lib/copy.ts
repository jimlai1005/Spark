...
export const COPY_ZH = {
  ...
  explore: {
    ...
    // R4-3：期間鍵對映 HL 實際回傳的窗（day/week/month/allTime），不是 90 天
    // ——Hyperliquid portfolio() 沒有 90 天窗。
    windows: { day: "1D", week: "7D", month: "30D", allTime: "全部" },
    poolNote: "候選池以 30D 表現選出；切換期間只改變顯示與排序，不重選候選池。",
    filters: {
      minLiveDaysLabel: "最少實盤天數",
      minFillsLabel: "最少成交筆數",
      maxDdPctLabel: "最大回撤上限 %",
      maxConcentrationPctLabel: "最大單一幣種集中度上限 %",
      clearToDisable: "清空即不套用此條件",
    },
    countPrefix: "",
    countMid: " 個帳戶 → 符合 ",
    countSuffix: "",
...
export const COPY_EN: DeepString<typeof COPY_ZH> = {
  ...
  explore: {
    ...
    poolNote: "The candidate pool is selected from 30D performance; switching the period only changes what's shown and how it's sorted, not the pool.",
    filters: {
      minLiveDaysLabel: "Min live days",
      minFillsLabel: "Min fills",
      maxDdPctLabel: "Max drawdown cap %",
      maxConcentrationPctLabel: "Max single-coin concentration cap %",
      clearToDisable: "Clear to disable this filter",
    },
    countPrefix: "",
    countMid: " accounts → ",
    countSuffix: " qualify",
