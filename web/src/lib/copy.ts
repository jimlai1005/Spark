...
export const COPY = {
   ...
   * 3. 目前生效值來自 `/api/me/capital`（見下方 `current`）——**滑桿的位置不是現況**，
   *    本頁不拿它假裝是現況。查詢失敗時說「暫時讀不到」，不退化成「你沒有設定」。
   *    <!-- 2026-07-27: 端點已上線，原註解「後端沒有查詢端點」已過時，改寫。 -->
   *    （確認框的「目前值」仍標為無法顯示：那一格要的是與待簽值同一時點的快照，
   *    面板的查詢是另一個時點，混用會讓前後對照的兩邊不同源。）
  ...
  capital: {
     ...
     * - 查詢失敗（503／網路）→ 說「暫時讀不到」，**絕不**畫成「你沒有設定」：後者是一句
     *   我們沒有根據的斷言（讀不到 ≠ 沒有，工程原則 3）。且本區失敗**不得影響頁面其餘部分**
     *   ——所以它是獨立的元件與獨立的 query。
     * - `effective` 存在時展示實際生效值與來源；`null` 時（env 預設）也要顯示預設來源。
     * - `pending` 存在時要**單獨一個區塊**標示「已提交、尚未生效」，**絕不能**讀起來像
     *   已經生效了（調低比例的客戶會以為曝險已經降下來而在錯誤的安全感下加碼）。
     * - 狀態的**內文**一律用後端 `note` 原文（單一來源）；本頁只提供標題與欄位標籤。
     */
    current: {
      title: "你目前生效的資金設定",
      loading: "載入中…",
      failedTitle: "暫時讀不到你目前的設定",
      failedNote: "無法查詢後端（可能是網路暫時中斷或伺服器暫時無法回應），本次暫時看不到。頁面其餘部分仍可正常使用。",
      statusLabel: "狀態",
      effectiveLabel: "目前生效值",
      allocatedLabel: "投入本金",
      utilizationLabel: "使用比例",
      sourceLabel: "來源",
      sourceCustomer: "你簽署的設定",
      sourceEnvDefault: "系統預設（尚未簽署過變更）",
      changedAtLabel: "上次變更時刻",
      // ⭐ 有用到：`changed_at === null` 時整列**照樣顯示**、值寫「（尚未變更）」。
      // 整列隱藏會讓 env 預設帳號分不出「從未變更」與「這個欄位這次沒讀到」。
      changedAtNull: "（尚未變更）",
      asOfLabel: "此值查詢於",
      /**
       * ⭐ `use_full_equity=true` 的本金顯示。後端是**硬不變量**（copytrade/config.py
       * 的 `__post_init__`）：這個旗標為真時 `allocated_capital` 必須是 0——那個 0 是
       * 「不准有值」，不是「本金為零」。把它原樣畫成「0.00 USDC」，再配上上方術語表
       * 教的「本金 × 比例 = 部位基準」，客戶會算出曝險為零，而引擎其實是拿整個帳戶
       * 權益在下單。這是本頁最不能出的錯，所以本金那一格改印這句話。
       */
      fullEquityValue: "使用全部帳戶權益",
      fullEquityNote:
        "這個帳號沒有設定固定的投入本金，引擎以你帳戶當下的全部權益為基準，"
        + "再乘上下方的使用比例來決定部位規模。權益變動時，部位規模會跟著變動。",
      // ⭐ pending 標題依 `state` 分兩種：後端刻意把兩態分開（app.py），因為處置不同。
      // `not_yet_applied`＝確定還沒套用；`unconfirmed`＝無從得知套用了沒。
      pendingTitleNotYetApplied: "已提交、尚未生效的變更",
      pendingTitleUnconfirmed: "已提交的變更，目前無從確認是否已生效",
      // 後端送來第三種 state 時的保底（不靜默落進上面任何一種）。
      pendingTitleFallback: "已提交的變更（狀態碼陌生）",
      // <!-- 2026-07-27: 刪掉未使用的 `pendingLabel`（"提交內容"）。pending 區塊是一份
      // 逐欄的 <dl>，每一欄已有自己的 dt，再加一個總標籤只是多一層沒有指涉對象的字。 -->
      pendingStateLabel: "狀態",
      pendingStateNotYetApplied: "等待引擎套用",
      pendingStateUnconfirmed: "待驗證",
      pendingSubmittedAtLabel: "提交時刻",
      pendingEffectiveWhenLabel: "預計生效於",
      /**
       * ⭐ `status === "unknown"` 是後端**明訂**的一態（已活化、但心跳缺席或過期），
       * 與「前端不認識的狀態碼」是兩件事，文案不得共用一句：合併之後畫面會在標題
       * 「你目前生效的資金設定」底下寫「未知的狀態碼」，而面板裡唯一的數字是 pending
       * 那組——客戶會把待生效值讀成現況。
       */
      unknownTitle: "引擎心跳缺席或過期，暫時無法確認目前生效的值",
      // 後端 status 值未知時的保底標題；狀態碼另以 statusLabel 那行原樣顯示。
      unknownStatus: "未知的狀態碼",
      notActivatedTitle: "此帳號尚未活化",
      indeterminateTitle: "帳號狀態無法確認",
    },
