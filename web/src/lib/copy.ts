...
export const COPY = {
  ...
  wizard: {
    ...
    // ⚠️ 2026-07-30 修正（審查 F6）：原文寫「系統以 perp 帳戶淨值計算回撤保護」，
    // 但風控**預設不啟用**（錢包主人在跟單頁自選）。對一個沒開風控的客戶宣稱有
    // 回撤保護，方向是讓他高估保護並據此放大本金——所以把保護那半句改成條件句。
    // 轉出資金的提醒本身仍然無條件成立：perp 淨值同時是跟單規模的分母。
    fundsWarning:
      "跟單期間請勿將資金從永續合約（perp）帳戶轉出。系統以 perp 帳戶淨值計算跟單規模；" +
      "**若你已啟用風控**，轉出資金會被視為虧損，可能觸發保護性平倉。" +
      "若要調整資金請先在此頁停止跟單。",
    errors: {
  ...
  leaders: {
    ...
    // ⭐ 安全揭露：跟單期間轉出 perp 資金會被回撤保護誤判為虧損，可能觸發保護性
    // 平倉。貼在本頁（客戶查看與管理跟單狀態的地方），與 wizard 開通頁的同義句
    // （COPY.wizard.fundsWarning）各自成立、互不取代。
    // ⚠️ 2026-07-30 修正（審查 F6）：同 wizard.fundsWarning——風控預設不啟用，
    // 所以「回撤保護」那半句必須是條件句，否則本頁上方的風控區塊（說預設不介入）
    // 與這一行會在同一個畫面上互相打對台。
    fundsWarning:
      "提醒：跟單期間請勿將資金從永續合約（perp）帳戶轉出——系統以 perp 淨值計算跟單規模；"
      + "若你已啟用風控，轉出會被視為虧損並可能觸發保護性平倉。",
    /**
    ...
    risk: {
      ...
      // ⭐ 時序提醒（審查觀察 4）：引擎在你選定 leader 後約一分鐘就會啟用，而風控
      // 設定在啟用的那一刻固定下來——先選 leader 再回來調的客戶會拿到「已鎖定」。
      saveFirstNote:
        "請在選定 leader 之前先儲存風控設定：引擎會在你選定 leader 後約一分鐘啟用，"
        + "屆時這份設定就會固定下來。",
      detailsTitle: "細項參數",
      percentSuffix: "%",
      saveButton: "儲存風控設定",
      saving: "儲存中…",
      saved: "風控設定已儲存，會在引擎啟用時生效。",
      loadError: "風控設定暫時讀不到（不影響你選 leader）。請稍後重新載入本頁。",
      saveError: "風控設定儲存失敗",
      // 已啟用的帳號：表單關閉並說明原因（後端 not_editable_note 也會附一份）。
      lockedTitle: "風控設定已鎖定",
    },
    // ⭐ 誠信要求：必須貼著送出按鈕出現。
    upperBound:
