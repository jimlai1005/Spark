...
def test_copytrade_e2e_testnet():
    ...
    resting_order = Order(coin=settings.coin, is_buy=True, size=settings.order_size,
                          limit_px=mid * Decimal("0.7"), tif="Gtc")
    ...
    modify_target = Order(coin=settings.coin, is_buy=True, size=settings.order_size,
                          limit_px=mid * Decimal("1.05"), tif="Ioc", reduce_only=False)
    ...
    try:
        ...
        print(f"modify 路徑 accrual 增量 {delta_modify}（> 0，支持 T1.3 假說 (a) 歸屬保留）")
        ...
        print(f"警告（非 fail，T1.3 政策資料點——modify 路徑 accrual 增量為 0）：{e}\n"
             "判讀見 docs/superpowers/research/2026-07-16-modify-builder-attribution.md 第 3 節。")

    # 4. place 路徑 accrual：baseline_2 = 當前 accrued → marketable Ioc（帶 builder，
    #    走 orchestrator.place_marketable_order，builder 經 "order" action，已知歸屬正確）
    #    → wait_for_accrual(baseline_2) 斷言增量 > 0。
