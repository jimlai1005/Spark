...
- 前端（2026-08-29 改版後）：文案雙語單一來源 `web/src/lib/copy.ts`（COPY_ZH/COPY_EN 結構對稱、元件經 `useCopy()` 取用，禁內嵌中文；法務長文在 `web/src/content/legal.ts` 同紀律）。公開路由 `/ /strategies /strategies/[slug] /leaderboard /traders/[address] /advanced /docs /terms /privacy /risk /status`（/docs 自 2026-08-29 起不在 header/footer 導覽，路由仍在）；登入後 `/onboarding /dashboard /settings`。對外主機名走 build 期 `NEXT_PUBLIC_SITE_ORIGIN`（RUNBOOK §4.2）。
