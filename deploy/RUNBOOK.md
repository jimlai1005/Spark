...
最長等待 1.82 小時、六種 job 最老到期皆有界、探測佔 fills 類 5.8%——修前（7.9b）同一模擬 1 天內錯誤重掃 2,174 次、核驗零推進。
短於 1 小時（`FILET_EXPLORE_FILLS_PERIOD_S` 啟動時驗證 ≥ 3600）在任何多頁增量情境下都會立即觸頂。
部署後以取樣器 v3 的 `scan`／`evidence`／`inc_freshness`／`cross_round` 欄位對照本算式：`fills_verify` 積壓應單調遞減、
`evidence_unknown` 只因核驗完成而減少、`fills_scan` 列數增速 ≈ 新增候選＋partial 重掃（不得每輪成長）。
...
類別感知領工（偏好類別領不到同 tick 改領另一類）；等待加權（每 10 分鐘升一級）；`BudgetExhausted` 保留原到期時間；health `explore_budget_note`／
`explore_refresh.fills_pages_total`；另含 7.1（`fills_coverage.synced_through`／`last_success_at`）與 7.2（策略頁／詳情頁 null 不轉 0）。
新 drop-in 兩行：`FILET_HL_EXPLORE_BASE_WEIGHT_CAP=180`、`FILET_HL_EXPLORE_FILLS_WEIGHT_CAP=120`；快照備份 `explore_index.json.pre-74.bak`。
