...
5. **（2026-08-29 追加）60 天上架閘門拔除**：使用者裁決推翻設計稿 NOTE 04——免責已講清楚、
   平台本就不審查跟單來源（進階模式即明證）。`listable = enabled and accepting_new`，
   不再看 covered_days；首頁「上架門檻 ≥ 60 天」文案移除。統計層的 RATIO_MIN_DAYS=60
   （Sharpe 樣本不足顯示）**保留**——那是誠實統計，不是准入。
6. **（2026-08-29 追加）onboarding 已跟單者短路**：已是 active follower（/api/me/leader
   status=following）進 /onboarding 且所選策略＝目前 leader → 顯示「已在跟單此策略」
   面板＋前往 Dashboard，不進綁定流程；step 2「完成綁定」失敗訊息改逐條列出未滿足
   條件（agent 授權／builder fee／入金），不再用籠統紅字。
