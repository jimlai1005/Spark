...
| 18:45 | 0 | 0 | 200／82／18 | — | 23 | 101／0 | ok | pages 2；fills_verify due 6（p95 1098s） |
| 19:00 | 0 | 0 | 204／79／17 | — | 23 | 98／1 | ok | pages 1 |
| 19:15 | 0 | 0 | 209／75／16 | — | 23 | —／0 | ok | pages 0；fills_verify due 7（p95 1894s） |
| 19:30 | 0 | 0 | **210**／75／15 | — | 22 | 91／0 | ok | pages 0 |

**19:36 排程檢查（+3h39m）**：池內 unknown **89 → 79**（2h 累計 100 → 79＝−21，門檻 −20 ✓）、池內 truncation_suspected 8 → **7**
（個位數 ✓）、earlier_fills_seen 185 → 194、no_earlier_activity 18 → 20；池內 complete 201 → **212**、partial 82 → 74；全體 complete
287 → 298；對外 complete 200 → **210**、eligible 83；探測落地最近 1h 14、累計 70；due fills_scan 89、running 147 → 135；隔離空；
`fills_verify` 22（due 6–7、p95 最高 1894s——輔助份額在探測與核驗間輪流，核驗等待拉長屬預期，backlog 仍在降）；
取樣器的 `scan_pages_15m`／`pages_consumed_15m` 讀 0–2 但 fills 本小時 +25k（1,859,935 → 1,884,942）、探測 14 筆落地——
...
| 20:45 | 0 | 0 | 218／70／12 | — | 22 | 80／0 | ok | pages 4 |
| 21:00 | 0 | 0 | 217／70／13 | — | 22 | —／1 | ok | pages 10 |
| 21:15 | 0 | 0 | 223／64／13 | — | 20 | 73／0 | hb 59s、err 0 | pages 4；單筆心跳齡 59s（下一筆回 1s，unit 未動） |
| 21:30 | 0 | 0 | **225**／62／13 | — | 19 | 70／0 | ok | pages 4 |

**21:36 排程檢查（+5h39m）**：池內 unknown **74 → 63**（2h 累計 79 → 63＝−16，仍低於 −20 但持續單調下降、未探完）、
池內 truncation_suspected 6 → **8**（個位數 ✓；6 小時窗至 21:57，將以個位數收尾）、earlier_fills_seen 200 → 210；
池內 complete 217 → **226**、partial 70 → 62；全體 complete 302 → 314；對外 complete 216 → **225**、eligible 84；探測落地最近 1h 14、
累計 90；due fills_scan 83 → 70、running 129 → 116（遍歷陸續完成）；fills +23k（1,936,876 → 1,960,305）；隔離空；
...
| 23:45 | 0 | 0 | 240／44／16 | — | 14 | —／0 | ok | pages 7 |
| 00:00 | 0 | 0 | 234／43／23 | — | 14 | —／11 | ok | pages 13；池輪替（backfilling 16 → 23） |
| 00:15 | 0 | 0 | 239／40／21 | — | 14 | 56／0 | ok | pages 1 |
| 00:30 | 0 | 0 | **243**／38／19 | — | 14 | —／0 | ok | pages 2 |

**00:36 排程檢查（+8h39m）**：池內 unknown **49 → 46**（本小時池內探測落地 14、新進池位址 9，淨 −3；2h 累計 −3，遠低於 −20，
但原因是輪替進出而非探測停擺——探測節奏 14/h 未變）、池內 truncation_suspected **8**（不變）、earlier_fills_seen 224 → 230；
池內 complete 241 → **244**、partial 43 → 38、backfilling 16 → 18；全體 complete 339 → 351；對外 complete 240 → 243、eligible 84；
探測累計 130；due fills_scan 54 → 51、running 101 → 99；fills +31k（2,036,600 → 2,067,710）；隔離空；`fills_verify` 14；
Traceback 0；429 0；follower 不變（03:21:18 09-22／05:21:18 09-18）；failed 0；timers 4；cron.err／host_cron.err 0。
主機：cpu 18–23%、api cgroup 956–995 MB、available 1,075 MB、swap 306 MB。
...
| 01:45 | 0 | 0 | 256／27／17 | — | 11 | —／0 | ok | pages 2；eligible 88 |
| 02:00 | 0 | 0 | 261／25／14 | — | 11 | 32／0 | ok | pages 2；eligible 92 |
| 02:15 | 0 | 0 | 263／24／13 | — | 11 | 30／0 | ok | pages 2 |
| 02:30 | 0 | 0 | **269**／19／12 | — | 10 | 24／0 | ok | pages 2；eligible 93 |

**02:36 排程檢查（+10h39m）**：池內 unknown **34 → 20**（探測落地 15、新進池 0；2h 累計 46 → 20＝−26 ✓）、
池內 truncation_suspected 8 → **7**、earlier_fills_seen 241 → 256；池內 complete 257 → **271**（90%）、partial 27 → 18、backfilling 16 → 11；
全體 complete 368 → 382；對外 complete 254 → **269**、eligible 93；探測累計 162；due fills_scan 37 → 23、running 85 → 71；
...
| 06:45 | 0 | 0 | 294／5／1 | — | 0 | 0／0 | ok | pages 1 |
| 07:00 | 0 | 0 | 294／5／1 | — | 0 | 2／1 | ok | pages 2 |
| 07:15 | 0 | 0 | 294／5／1 | — | 0 | 0／0 | ok | pages 2；eligible 93 |
| 07:30 | 0 | 0 | 294／5／1 | 0／5 | 0 | 0／0 | ok | pages 0 |

**07:36 排程檢查（+15h39m）**：穩態不變——池內 unknown **0**、truncation_suspected **5**、earlier_fills_seen 277、no_earlier_activity 18；
池內 **complete 295／partial 5**；全體 complete 414；對外 complete 294、eligible 93；新進池 1、探測落地 2；due fills_scan 0、
running 42、`fills_verify` 0；fills +1.5k；隔離空；Traceback 0；429 0；follower 不變（03:21:18 09-22／05:21:18 09-18）；failed 0；
timers 4；cron.err／host_cron.err 0。主機：cpu 13–15%、api cgroup 997–1,068 MB、available 1,083 MB、swap 307 MB。
**判定：安全面正常，不回退。**

| 07:45 | 0 | 0 | 294／5／1 | — | 0 | 0／0 | ok | pages 0 |
| 08:00 | 0 | 0 | 294／5／1 | — | 0 | 0／0 | ok | pages 2 |
| 08:15 | 0 | 0 | 294／5／1 | — | 0 | 0／0 | ok | pages 0；eligible 91 |
| 08:30 | 0 | 0 | 294／5／1 | 0／5 | 0 | 0／0 | ok | pages 3 |

**08:36 排程檢查（+16h39m）**：穩態不變——池內 unknown **0**、truncation_suspected **5**、earlier_fills_seen 276、no_earlier_activity 19；
...
| 09:45 | 0 | 0 | 294／5／1 | — | 0 | 0／0 | ok | pages 6 |
| 10:00 | 0 | 0 | 294／5／1 | — | 0 | 0／9 | ok | pages 15 |
| 10:15 | 0 | 0 | 289／5／6 | — | 0 | 5／0 | ok | pages 3；池輪替進 8 個 |
| 10:30 | 0 | 0 | 291／5／4 | 1／5 | 0 | 3／0 | ok | pages 5 |

**10:36 排程檢查（+18h39m）**：10:00 池輪替進 8 個新位址 → 探測落地 8（進池即探，D-O＋全史窗生效）、池內 unknown 只剩 **1**
（backfilling 1，等下一頁）、truncation_suspected **5**、earlier_fills_seen 276、no_earlier_activity 18；池內 complete 294／partial 5；
...
| 11:45 | 0 | 0 | 292／6／2 | — | 0 | 1／0 | ok | pages 9 |
| 12:00 | 0 | 0 | 293／6／1 | — | 0 | 0／0 | ok | pages 11 |
| 12:15 | 0 | 0 | 293／6／1 | — | 0 | 0／0 | ok | pages 10 |
| 12:30 | 0 | 0 | 293／6／1 | 0／6 | 0 | 0／0 | ok | pages 10；eligible 92 |

**12:36 排程檢查（+20h39m）**：池內 unknown **0**（上小時 3 個已探完＋新進池 1 個即探）、truncation_suspected 5 → **6**（個位數 ✓）、
earlier_fills_seen 277、no_earlier_activity 17；池內 **complete 294／partial 6**；全體 complete 429；對外 complete 293、eligible 92；
...
| 13:45 | 0 | 0 | 292／7／1 | — | 0 | 0／0 | ok | pages 5 |
| 14:00 | 0 | 0 | 292／7／1 | — | 0 | 0／0 | ok | pages 7 |
| 14:15 | 0 | 0 | 293／6／1 | — | 0 | 0／0 | ok | pages 9；eligible 92 |
| 14:30 | 0 | 0 | 293／6／1 | 0／6 | 0 | 0／0 | ok | pages 8 |

**14:36 排程檢查（+22h39m）**：池內 unknown **0**（新進池 2、探測落地 3）、truncation_suspected 7 → **6**、earlier_fills_seen 276、
no_earlier_activity 18；池內 **complete 294／partial 6**；全體 complete 432；對外 complete 293、eligible 92；due fills_scan 0、
