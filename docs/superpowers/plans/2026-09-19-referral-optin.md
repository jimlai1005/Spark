...
- 2026-09-21（19:21 UTC 09-20）：**推薦碼換成 `FILET`**（使用者指示），推薦人錢包改為 Filet Alpha `0xfB9C…9760`（主網 `referral` 端點 `referrerState.stage == ready`、`code == FILET`）。程式碼零改動：只改正式機 `/etc/filet/referral.env`＋重啟 `filet-api`（watcher 是 timer oneshot，下一輪自然吃到）。舊值備份 `/etc/filet/referral.env.bak-20260920-192110`。已知殘留：`referral_optin.json` 內有一筆 2026-09-20 以舊碼簽的記錄（account `ffb9c5…9760`，即 Filet Alpha 自己，鏈上已是 `RABBYWALLET`，無 follower 在跑）——若日後該帳號啟用引擎會觸發 `referral_code_mismatch` critical（`referral_apply.py` 記錄碼比對先於鏈上查詢），需請該帳號重簽。
