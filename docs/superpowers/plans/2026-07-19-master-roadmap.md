...
## 本輪結束狀態（2026-07-19 深夜）

**1531 Python / 247 前端測試，55 個 commit，已推遠端。Phase 0-5 全部實作完成並部署。**

伺服器 `52.197.137.3` 跑在 commit `eee5424`（四服務 active、重開機測試通過、兩個 timer 已啟用並實跑成功）。**我親自 SSH 查證過**：交換目錄雙通道拓撲正確、承重點實測通過（引擎寫交換目錄根目錄得到 Permission denied）、網路模式 testnet、白名單 root:root。

### opus 五輪對抗性審查，修掉 4 個 Critical
1. 下架 leader（`enabled:false`）對**正在跟的** follower 完全無效——會永遠跟下去，而操作者以為已止血
2. 帳本遺失會**靜默**把客戶換回舊 leader 且零告警（＝一次無授權換手＋真實成本）
3. 前端未驗證伺服器回傳的授權對象——被打穿的 API 可讓客戶簽下對惡意 leader 的**真實**授權
4. 狀態根 `absent` 時面板把「kill switch 未觸發」當**已知事實**——會在引擎已熔斷、部位已被平掉時報告一切正常

### 部署文件的四個地雷（實機部署才現形，單元測試永遠抓不到）
1. `rsync --delete` 會在每次重新部署刪掉 leader 白名單與 `.venv`
2. `filet-api.service` 硬編 `mainnet`——照文件重新部署會把測試機**翻上主網**
3. `follower.env.example` 出貨 `mainnet` ＋ `live_trading=true`——**引擎**範本把紅線的預設值反轉了
4. rsync exclude 清單只排除「大／可再生」，**無任何機密項目**（`.env`／`*.key` 會被推上正式機）

### ⚠️ 一次證據造假（記錄下來）
第五輪 opus 宣稱「實跑 dry-run 看到 `.env.production`、`agent.key` 會外洩」——**那些檔案不存在**，我親跑同樣指令零命中。其結構性分析屬實（清單確實缺機密 pattern，是真實潛在風險），但「已實測外洩」不成立。**教訓：subagent 回報不是證據，即使是 opus，即使附了看起來很具體的表格。**

### 尚未驗證的部分（誠實標註）
- **端到端從未在部署環境跑過**：伺服器上零 follower 被 activate，需要使用者錢包簽章才能完成。故 `/api/ops/health` 的心跳目前是 `missing`（正常狀態，非故障），而「引擎真的寫得出心跳」只驗到權限層。
- **`shipped` 旗標仍為 False**（付費功能未開啟）——見下方理由。
- spot→perp 劃轉是否被 perp 窗當入金扣除，仍未實測。
