...
| 7.9d-D | 2026-09-22 | 5c1b3b4＋2778e56 | 刪毫秒常數；同源彙總查詢分列版（rows／addresses／active／inactive／orphan）、`delete_inactive_jobs`、`scan_job_targets`；遷移閘門 docstring；非法頁不終止輪次、`complete_scan` 拒絕 result=None。正式機快照：`fills_verify` {129,129,112,17}、running 69／orphan 12 |
| 7.9d-S | 2026-09-22 | 8831282＋8f7a0d4 | 狀態與工作對帳（啟動＋每次 candidates；resume 依 scan kind；冪等）；退池發送前丟棄＋主動掃除；輔助份額以頁面計（verify／probe 輪流、逾期只調順序、續頁不重設等待）；相容層拆除（介面不符 TypeError 逸出）；status 同母體分列；門檻五測；3 個變異驗證命中 |
| 主線程整合小修 | 2026-09-22 | 1a9e7e9 | 詳情頁只對 active 候選按需入列、準入常數同源；非法頁指數退避＋達上限隔離（`invalid_pages`） |
| **7.9d 最終整合對帳（主線程）** | 2026-09-22 | 8f7a0d4 | ruff 乾淨；pytest 3292；vitest 761；正式機 287 列快照遷移三次（含 version 回 2＋重啟）一致；`repro_rescan`（3 天 scans=1／滿頁 3）、`repro_79b`、`rv_churn_backfill2`（兩種 scan 掉池回池續跑同 scan_id 並完成）、`rv_verify_burst`（每 10 頁 1 次）PASS；3 天模擬（churn 20 址、多頁遍歷、每分鐘一次預留）：partial_rescan 3、verify 112/112 各恰一次（17 件退池掃除）、六 kind 最老到期 < 0.4h、探測 5.7%、churn 後無卡住。**未部署，複審中** |
| 7.9d 複審→7.9e | 2026-09-22 | 36763ba（D）、c7ee420（主線程 app import）、024e442（S） | 複審（opus）2 Critical 同根（verify job 被 kind 閘門刪／退池後不重建 → 證據永久缺失）＋2 Warning；修：第四原因碼 `verify_needed`（`_ensure_scan_job` 單一函式）、verify job 延後不刪、`scan_job_dropped_kind_mismatch` 分計、對帳例外不隔離 candidates、輔助名額只在真發請求時歸零、逾期 verify 與 probe 輪流、`_run_cache_kind` 發送前 active、孤兒判準對 kind、`verify_needed` 彙總、`ADMISSION_MULTIPLIER` 搬資料層；3 條變異驗證命中 |
| **7.9e 最終整合對帳（主線程）** | 2026-09-22 | 024e442 | ruff 乾淨；scheduler 測試三跑皆 87 passed（無 flaky）；pytest 3306；vitest 761；相容層／毫秒常數 grep 零命中；快照遷移三次（version 回 2、重啟）一致；`repro_rescan`／`repro_79b`／`rv_churn_backfill2`／`rv_verify_burst` PASS，複審的 `rv_verify_loss` test_A 反轉（verify job 不再消失）；3 天模擬（churn 20 址、多頁遍歷、每分鐘一次預留）：partial_rescan 3、verify 112/112 各恰一次、六 kind 最老到期 < 0.4h、探測 5.9%、churn 後無卡住、**active 地址 evidence_unknown 0／需核驗無工作 0**。**未部署，複審中** |
| 7.9e 複審→修正 | 2026-09-22 | c05ea3c（主線程 W1）、e09cbe3（S W2/W3） | 複審（opus）：**可部署，先修 W1**（`verify_needed.unserved` 與排程不同源）；W2 延後的 verify job 證據已清仍開遍歷、W3 延後路徑燒名額、W4 D4 死欄位（未砍，下一批）。修後：全量 3309；harness 釘種子後 3 天模擬全項 PASS（verify 109/109 各恰一次，3 次因證據已補而省略）。**W2/W3 修正未經獨立複審**（小改動、各有守門測試與變異驗證） |
| **第七次部署（7.9a–7.9e）** | 2026-09-22 04:47 UTC | 2048cd0（程式 e09cbe3） | 使用者裁決五項證據齊即受控部署。rsync → import → build → chown → DB 備份 `.pre-79.bak` → drop-in `FILET_EXPLORE_FILLS_PERIOD_S=21600` → 取樣器 v3 → restart api＋dashboard → schema 3、349↔349、running 56、unknown 163、gap 0、verify job 131 攤 48h、對帳掃除 32 筆、零 Traceback → `DEPLOYED_VERSION`。觀測：至少一個完整 6h 週期 |
