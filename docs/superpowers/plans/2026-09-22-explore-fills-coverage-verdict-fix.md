...
| 07:00 | 0 | 0 | 137／147／16 | 37 | 0／0／1 | ok | pages 0（無到期 job，遍歷軌閒置） |
| 07:15 | 0 | 0 | 137／147／16 | 36 | 0／0／0 | hb 60s（單筆） | |
| 07:30 | 0 | 0 | 137／147／16 | 36 | 0／0／0 | ok | |
| 07:45 | 0 | 0 | **138**／147／15 | 36 | 0／0／0 | ok | |

**07:55 排程檢查（+16h10m）**：unknown 88 → 86（池內 15）、`earlier_fills_seen` 194 → 196、`no_earlier_activity` 24 → 25、
`truncation_suspected` 158 → 159；completeness 57／205／204；`fills_verify` job 37 → 35；running scans 59 → 57；
...
| 10:00 | 0 | 0 | 141／147／12 | 32 | 1／2／0 | ok | pages 7 |
| 10:15 | 0 | 0 | 141／148／11 | 32 | 0／2／0 | ok | pages 6 |
| 10:30 | 0 | 0 | 142／148／10 | 32 | 2／1／0 | ok | pages 6 |
| 10:45 | 0 | 0 | **143**／148／9 | 32 | 0／0／0 | ok | pages 9；eligible 87 |

**10:55 排程檢查（+19h10m）**：unknown 82 → 78（池內 10）、`earlier_fills_seen` 200 → 203、`no_earlier_activity` 25、
`truncation_suspected` 162 → 163；completeness 50／211／208；`fills_verify` job 32；running scans 53 → 50；
Traceback 0；429 0；follower 不變；cron.err 0。**判定：安全面正常，不回退。**

| 11:00 | 0 | 0 | 143／146／11 | 32 | 1／2／2 | ok | pages 9；eligible 88 |
| 11:15 | 0 | 0 | 143／146／11 | 31 | 0／1／2 | ok | pages 10 |
| 11:30 | 0 | 0 | 145／146／9 | 31 | 0／3／1 | ok | pages 10 |
| 11:45 | 0 | 0 | **146**／146／8 | 31 | 0／3／0 | ok | pages 12 |

**11:55 排程檢查（+20h10m）**：unknown 78 → 76（池內 11）、`earlier_fills_seen` 203 → 205、`no_earlier_activity` 25 → 26、
`truncation_suspected` 163 → 164；completeness 49／214／208；`fills_verify` job 31；running scans 50 → 49；
對外 complete 143 → 146、eligible 88；Traceback 0；429 0；follower 不變（03:21:18 09-22／05:21:18 09-18）；
failed 0；cron.err 0。**判定：安全面正常，不回退。**

| 12:00 | 0 | 0 | 145／144／11 | 31 | 3／3／1 | ok | pages 14；overdue p95 fills 92s／fills_scan 79s／ledger 3s；eligible 87 |
| 12:15 | 0 | 0 | 146／145／9 | 30 | 1／2／0 | ok | pages 9 |
| 12:30 | 0 | 0 | 147／145／8 | 30 | 2／1／3 | ok | pages 9 |
| 12:45 | 0 | 0 | **149**／145／6 | 30 | 0／0／0 | ok | pages 7 |

**12:55 排程檢查（+21h10m）**：unknown 76 → 74（池內 8）、`earlier_fills_seen` 205 → 209、`no_earlier_activity` 26 → 27、
`truncation_suspected` 164；completeness 46／219／209；`fills_verify` job 30；running scans 49 → 46；
