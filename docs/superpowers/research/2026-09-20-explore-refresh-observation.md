...
- 結論：新增速度（~1/h × 1.2 頁）遠低於消化（60 頁/h）；剩餘第一輪積壓＝原候選未開始 95＋新增未開始 71＝166 個地址 × ~1.2 頁 ≈ 200 頁 ≈ 3.5 小時；增量輪（每 4 小時、多為短頁且 settle 後只佔實際權重）預期不會再造成積壓。**長期可收斂**；若候選輪動突然升到每小時 >40 個才會逼近上限。
...
三者都是連續滿頁後以短頁收尾、去重後筆數與 HL 完全一致；`fills_in_window` 最高 7,992 < 8,000 留存門檻，判 complete 成立（30 天內筆數 < 10,000 ⇒ 區間內成交必在可查範圍）。附帶：本機第一次未帶限流器直抓即 429，第二次帶 `WeightLimiter` 零 429。
