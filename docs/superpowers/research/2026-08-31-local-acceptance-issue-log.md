...
| 裁決 B | `/leaderboard` 轉址形態 | **使用者裁決：改伺服器層永久轉址**。`next.config.ts` redirects() → 308 `/explore`；移除 client-side 轉發頁；首頁「全部策略 →」直連 `/explore`；Playwright 新增 308＋Location 斷言；prod 部署 `e6cc2c9`（web rebuild＋dashboard restart），`curl -I https://trade.filet.app/leaderboard` → 308 Location `/explore`，regression_check `--http` 43/43 | `e6cc2c9` | ✅（prod 已驗） |
