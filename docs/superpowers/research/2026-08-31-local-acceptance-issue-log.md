...
|---|---|---|---|---|
| I-15 | Filet Alpha 9760 數據與鏈上/參考工具不符（總報酬 6% vs 20%、回撤 −19% vs −2.4%、勝率 18% vs 68%…） | 我方用 `perpAllTime`（**僅 perp 帳戶**）序列算 TWR；這顆錢包資金停泊在 spot、經 spot↔perp 內部轉帳進出（鏈上 ledger 實證 07-01/07-21 sends）——轉帳被當成損益 → 假波動/假回撤。參考工具用 `allTime`（**spot+perp 合併**）序列：鏈上實測 1002.24→1197.9，與參考數字逐位吻合＝**參考工具是對的** | 策略績效管線改用合併 `allTime`/`month` 窗（此錢包 spot 只放 USDC，合併基準乾淨；「perp 基準」原裁決是為了可複製性，但未經流量調整的 perp 序列對這種錢包形態就是錯的——工程原則事故 #3 同型） | 🟥 |
...
| I-17 | explore 全關 filter 仍 100→27 | 27/24 是「enrich 成功且 month＋allTime 兩窗有效」的地址數；其餘 ~73 是上游資料缺席（新帳戶、抓取失敗/429、窗無效）被結構性跳過——與使用者 filter 無關。「顯示全部 100」= 大量整列「—」且無排序鍵 | 產品選擇題：(a) 維持現狀＋UI 註記「N 檔因鏈上資料不足未列入」；(b) 缺資料列附於榜尾標「資料不足」 | 🟥 |
| I-18 | 成交記錄 7天/BTC/UTC 選了但內容全不符，多按幾次會壞 | **現行 build 無法重現**：Playwright 對運行中 build 實測三 filter 逐一生效、快速連點 12 次穩定。程式碼審閱 cutoff/coin/tz 全正確接線。最可能：瀏覽器分頁跨越了多次本機重建（chunk 換 hash），載著**舊版 JS**（round-3 修 filter 前的版本正是「按鈕會亮但不過濾」）繼續跑 | 請先硬重新整理（Cmd+Shift+R）複測；若仍重現，我再加程式內診斷。可選加固：build-id 不符時自動整頁重載 | 🟥 |
